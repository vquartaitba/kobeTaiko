...
// Función para procesar la salida de la consola y extraer la dirección
async function extractContractAddress(consoleOutput) {
  const prompt = `Extract the contract address from the following Hardhat deployment output:

${consoleOutput}

Respond only with the contract address in the format: 0x...

if there are more than 1 contract address, answer only with the last one. remember to only answer with the address`;

  const contractAddress = await generateMessage(prompt, "gpt-4o");
  return contractAddress;
}

// Función para ejecutar el comando y manejar la salida de manera asíncrona
async function executeDeploymentCommand(command, cwd) {
  return new Promise((resolve, reject) => {
    exec(command, { cwd }, async (error, stdout, stderr) => {
      if (error) {
        ...
        const deployedAddress = await extractContractAddress(stdout);
...
// Función para desplegar en Base
async function deployToTaiko() {
  try {
    ...
    const prompt = `Given the following Solidity contract, generate the JavaScript code for Hardhat Ignition to deploy it:

${contractContent}

Here is the documentation of ignition:
...
To execute this Future, Hardhat Ignition won't send any transactions and it will use 
eth_call
instead. Just like every other Future, it gets executed once, and the result is recorded.
...
For example, we can modify the Apollo module from the Quick Start guide to make the name field in the Rocket smart contract configurable with a parameter:
...
In this case, the name of the contract is only used to generate 
Future
...
ANSWER ONLY WITH THE CODE TO PUT DIRECTLY IN THE FILE, WITH NOTHING ELSE
`;

    const generatedCode = await generateMessage(prompt, "gpt-4o");

    const outputPath = path.join(
      __dirname,
      "ignition",
      "modules",
      "generatedContract.js"
    );
    fs.writeFileSync(outputPath, generatedCode);

    console.log("Generated code successfully written to:", outputPath);

    const deployedAddress = await executeDeploymentCommand(
      "echo y| npx hardhat ignition deploy ./ignition/modules/generatedContract.js --network sepolia",
      __dirname
    );
    ...
    console.error("Error in deployToTaiko function:", error);
    return null;
  }
}
